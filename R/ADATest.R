#' Adaptive Combination of P-values (ADA) Test
#'
#' @name ADATest
#' @description The `ADATest` function implements the Adaptive Combination of P-values (ADA) algorithm.
...
#'
#' @param genotype A numeric matrix where rows represent samples and columns represent
#' variants. The entries should encode genotype information, with missing values indicated
#' as negative numbers.
#' @param phenotype A numeric vector representing the case-control status of samples.
#' Use `1` for cases and `0` for controls.
#' @param mafThr A numeric value specifying the minor allele frequency (MAF) threshold
#' for filtering variants. Defaults to `0.05`.
#' @param num_perm An integer specifying the number of permutations to perform for the
#' adaptive algorithm. Defaults to `1000`.
#' @param midp A logical value indicating whether to use the mid-p adjustment for the
#' hypergeometric test. Defaults to `FALSE`.
#' @param mode A character string specifying the genetic model. Options include
#' `'additive'`, `'dominant'`, and `'recessive'`. Defaults to `'additive'`.
#' @param twoSided A logical value indicating whether a two-sided test should be performed.
#' Defaults to `TRUE`.
#'
...
#'
#' @seealso `getPvalADA`, `getScoreADA`, `getScoreADAoriginal`
#'
#' @import data.table
#' @importFrom stats phyper dhyper dbeta
